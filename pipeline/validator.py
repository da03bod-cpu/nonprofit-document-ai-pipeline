...
def validate_output(data):
    ...
    allowed = {"type", "name", "year", "description", "beneficiaries_count", "budget", "beneficiary_value", "target_audience", "delivery_method", "notes"}
    cleaned = []
    for i, item in enumerate(programs):
        ...
        item["type"] = item.get("type") or "project"
        if item["type"] not in {"program", "project"}:
            item["type"] = "project"
        cleaned.append(item)
